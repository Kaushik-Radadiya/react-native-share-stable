...
https://github.com/react-native-community/react-native-share/issues/627


### Mostly automatic install
1. `yarn add react-native-share-stable`
2. `react-native link react-native-share-stable`

### Manual install

#### iOS Install

1. `npm install react-native-share-stable --save`
2. In XCode, in the project navigator, right click `Libraries` ➜ `Add Files to [your project's name]`
3. Go to `node_modules` ➜ `react-native-share-stable` ➜ `ios` and add `RNShare.xcodeproj`
...
3. Append the following lines to `android/settings.gradle`:
  	```
  	include ':react-native-share-stable'
  	project(':react-native-share-stable').projectDir = new File(rootProject.projectDir, 	'../node_modules/react-native-share-stable/android')
  	```
4. Insert the following lines inside the dependencies block in
   `android/app/build.gradle`:

    ```
      compile project(':react-native-share-stable')
    ```
...
1. `npm install react-native-share-stable --save`
2. In Visual Studio add the `RNShare.sln` in `node_modules/react-native-share-stable/windows/RNShare.sln` folder to their solution, reference from their app.
...
For eg.
```javascript
import Share from 'react-native-share-stable';
...
import Share, {ShareSheet, Button} from 'react-native-share-stable';
